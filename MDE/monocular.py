...
DEPTH_MODEL_TYPE = 'midas'      # Currently supports 'midas' (more models can be added)
TARGET_DISTANCE_M = 3.0         # Desired distance to target in meters
DISTANCE_DEAD_ZONE_M = 0.3      # Dead zone for distance control in meters
MAX_FORWARD_SPEED_MPS = 1.5     # Maximum forward/backward speed in m/s
YAW_CENTERED_THRESHOLD = 30     # Pixel threshold - when target is considered "centered" for forward movement
...
KI_DISTANCE = 0.05    # Integral gain for distance error
KD_DISTANCE = 0.15    # Derivative gain for distance error

# KALMAN FILTER GAINS (tune these!)
Q_KF = 0.1
R_KF = 10
# Distance Kalman filter gains
Q_DISTANCE_KF = 0.2   # Process noise for distance estimation
R_DISTANCE_KF = 0.5   # Measurement noise for distance estimation 
...
# --- Monocular Depth Estimator Class ---
class MonocularDepthEstimator:
    """
    Monocular depth estimation using MiDaS model for target distance calculation.
    This enables forward/backward movement control towards detected targets.
    """
    def __init__(self, model_type='midas'):
        ...
        print(f"Depth estimator will use device: {self.device}")
    ...
    def get_target_distance(self, frame, target_bbox, image_center_x, image_center_y):
        """Get estimated distance to target center in meters."""
        ...
        try:
            ...
            depth_x = int(target_center_x * w / frame_w)
            depth_y = int(target_center_y * h / frame_h)
            
            # Ensure coordinates are within bounds
            depth_x = max(0, min(w-1, depth_x))
            depth_y = max(0, min(h-1, depth_y))
            
            # Sample depth around target center (3x3 area for robustness)
            sample_size = 3
            x_start = max(0, depth_x - sample_size//2)
            x_end = min(w, depth_x + sample_size//2 + 1)
            y_start = max(0, depth_y - sample_size//2)
            y_end = min(h, depth_y + sample_size//2 + 1)
            
            depth_samples = depth_map[y_start:y_end, x_start:x_end]
            
            # Use median for robustness against outliers
            target_depth = np.median(depth_samples)
            
            # MiDaS outputs inverse depth, convert to approximate distance
            # This is a rough approximation - should be calibrated for your specific setup
            if target_depth > 0:
                estimated_distance = 10.0 / target_depth  # Rough conversion factor
...
async def tracking_loop():
    ...
    if _use_real_yolo_local: # Use the local flag here
        try:
            if REAL_YOLO_MODEL_PATH.endswith('.engine'):
                try:
                    if yolo_model.load_engine():
                        print(f"Real YOLO TensorRT engine loaded successfully from {REAL_YOLO_MODEL_PATH}")
                    else:
                        print("Failed to load YOLO TensorRT engine - falling back to mock YOLO")
                        raise RuntimeError("Failed to load YOLO TensorRT engine")
                except Exception as e:
                    print(f"Error loading YOLO TensorRT engine: {e}")
                    _use_real_yolo_local = False
                    yolo_model = None
            else:
                from ultralytics import YOLO
                yolo_model = YOLO(REAL_YOLO_MODEL_PATH)
                print("Real YOLO model loaded successfully with ultralytics")
        except ImportError:
        ...
        if current_flight_mode == FlightMode.OFFBOARD:
            ...
            if _use_real_yolo_local and yolo_model: # Use the local flag here
                try:
                    if REAL_YOLO_MODEL_PATH.endswith('.engine'):
                        #tensort inference
                        input_shape=yolo_model.get_input_shape()
                        resized_frame = cv2.resize(frame, (input_shape[2], input_shape[1]))
                        input_data = resized.astype(np.float32) / 255.0
                        input_data = np.transpose(input_data, (2, 0, 1))[None,:, :, :] # Add batch dimension
                        outputs = yolo_model.infer(input_data)
                        detections = post_process_yolo_output(outputs[0], input_shape, frame.shape)
                    else:
                        # ultralytics inference
                        results = yolo_model.predict(frame, verbose=Flase,conf=0.5)

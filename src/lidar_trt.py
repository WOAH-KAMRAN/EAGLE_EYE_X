# UAV Target Tracking System — LiDAR Distance Following
# Based on lastscr_trt.py with these additions:
#   - TF Mini LiDAR for distance measurement
#   - DistanceKalmanFilter + DistancePID
#   - Two-stage control: center target → read LiDAR → move forward
...
class TFMiniLidar:
    """
    TF Mini LiDAR interface.
    In real mode: reads serial 9-byte frames, parses distance in cm, converts to meters.
    In mock mode: returns simulated distance centered on TARGET_DISTANCE_M with noise.
    """
    ...
    def read_distance(self):
        """Returns distance in meters, or None if unavailable."""
    ...
    def _read_real(self):
        """Parse 9-byte TF Mini serial frame."""
        try:
    ...
    def _read_mock(self):
        ...
        When no prior reading: return a random distance near TARGET_DISTANCE_M.
        When target was recently seen: return last distance with small drift.
        """
...
def display_thread_func():
    ...
    cv2.namedWindow("UAV Tracking — LiDAR", cv2.WINDOW_NORMAL)
    try:
        cv2.setWindowProperty("UAV Tracking — LiDAR",
                              cv2.WND_PROP_OPENGL,
                              cv2.WINDOW_OPENGL)
    except Exception:
        pass
    while not display_should_stop.is_set():
        try:
            ...
            if frame is None:
                break
            cv2.imshow("UAV Tracking — LiDAR", frame)
...
async def tracking_loop():
    ...
    print("=" * 60)
    print("UAV TARGET TRACKING — LiDAR FOLLOWING")
    print("  YOLO + TF Mini LiDAR | Per-frame KF dt | Display thread")
    print("=" * 60)
    ...
    flight_dir = os.path.join(VIDEO_FOLDER, f"lidar_flight_{timestamp}")
        ...
        while not EMERGENCY_STOP:
            ...
            if mode == FlightMode.OFFBOARD:
                ...
                if target_centered and best_det is not None:
                    # Distance-following mode: use VelocityNedYaw
                    await drone.offboard.set_velocity_ned(VelocityNedYaw(
                        forward_velocity,   # North (+ = forward)
                        0.0,                # East
                        0.0,                # Down (altitude maintained by FC)
                        np.deg2rad(yaw_cmd) # Yaw rate
                    ))
                else:
                    # Centering / searching mode: use AttitudeRate
                ...
                cv2.putText(frame, f"DistKF: {kf_dist_m:.2f}m | Fwd: {forward_velocity:.2f}m/s",
                ...
                cv2.putText(frame, f"dt: {dt*1000:.0f}ms", (10, 145),

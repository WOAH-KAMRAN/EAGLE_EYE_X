...
#CONNECTION_STRING = 'serial:///dev/ttyACM0:921600'  # udpin://127.0.0.1:14550
CONNECTION_STRING = 'udpin://127.0.0.1:14550' 

YOLO_MODEL_PATH = 'best.pt'  # <--- UPDATE THIS!
...
async def preflight_checks(drone):
    ...
    try:
        ...
        if not health.is_gyrometer_calibration_ok:
            print("  ✗ Gyroscope not calibrated")
            checks_passed = False
    ...
    try:
        ...
        if gps.num_satellites < 8:
            print(f"  ⚠ Only {gps.num_satellites} satellites (need 8+)")
            checks_passed = False
        else:
            print(f"  ✓ GPS OK ({gps.num_satellites} satellites)")
    except asyncio.TimeoutError:
        print("  ✗ GPS check timeout")
        checks_passed = False
    ...
    try:
        battery = await asyncio.wait_for(
            drone.telemetry.battery().__anext__(),
            timeout=5.0
        )
        if battery.remaining_percent < 30:
            print(f"  ✗ Battery low: {battery.remaining_percent}%")
            checks_passed = False
        else:
            print(f"  ✓ Battery OK ({battery.remaining_percent}%)")
    except asyncio.TimeoutError:
        print("  ⚠ Battery check timeout (continuing)")
    ...
    if checks_passed:
        ...
        print("✗ Pre-flight checks FAILED - DO NOT FLY\n")

...
TAKEOFF_VELOCITY_MPS = -0.5    # Takeoff climb velocity in m/s (negative for upward) - Used for VelocityNedYaw command
...
ALTITUDE_CAP_M = 10.0          # Maximum altitude cap in meters
...
async def tracking_loop():
    print("Starting TensorRT-optimized tracking loop...")
    drone = System()
    ...
    takeoff_successful = await arm_and_takeoff(drone, 2.0)
    ...
    # --- TensorRT Model Initialization ---
    trt_engine = None
    model_input_shape = None
    
    _use_real_yolo_local = USE_REAL_YOLO and TRT_AVAILABLE
    
    if _use_real_yolo_local:
        try:
            print("Loading TensorRT engine...")
            trt_engine = TensorRTInference(REAL_YOLO_MODEL_PATH)
            if trt_engine.load_engine():
                model_input_shape = trt_engine.get_input_shape()
                print(f"TensorRT engine loaded successfully!")
                print(f"Model input shape: {model_input_shape}")
            else:
                print("Failed to load TensorRT engine. Falling back to mock YOLO.")
                _use_real_yolo_local = False
                trt_engine = None
        except Exception as e:
            print(f"Error initializing TensorRT engine: {e}. Falling back to mock YOLO.")
            _use_real_yolo_local = False
            trt_engine = None

    yaw_kalman_filter = KalmanFilter(dt=0.1, Q=Q_KF, R=R_KF, initial_pos=image_center_x) 
    pitch_kalman_filter = KalmanFilter(dt=0.1, Q=Q_KF, R=R_KF, initial_pos=image_center_y)
    ...
    csv_headers = ['Time_s', 'FlightMode', 'Raw_Yaw_Err_px', 'KF_Yaw_Est_px', 'Yaw_Cmd_deg_s', 'Yaw_I_term',
                   'Raw_Pitch_Align_Err_px', 'KF_Pitch_Align_Est_px', 'Pitch_Cmd_deg_s', 'Pitch_I_term',
                   'Target_Class_ID', 'Current_Altitude_m']
    ...
    while True:
        ...
        async for fm in drone.telemetry.flight_mode():
            current_flight_mode = fm
            break

        current_altitude_m = 0.0
        async for pos in drone.telemetry.position():
            current_altitude_m = pos.relative_altitude_m
            break

        log_data_row = [current_time, current_flight_mode.name]

        if current_flight_mode == FlightMode.OFFBOARD:
            if not is_offboard_active_from_script:
                try: await drone.offboard.start(); print("Offboard mode restarted."); is_offboard_active_from_script = True
                except OffboardError as error: print(f"Failed to restart offboard: {error}.")
            
            detections = []
            if _use_real_yolo_local and trt_engine:
                try:
                    # Preprocessing for TensorRT inference
                    ...
                    print(f"TensorRT inference error: {e}. Using mock detections.")
            ...
            for detection in detections:
                priority_score = CLASS_PRIORITIES.get(detection['cls'], -1)
                if priority_score > highest_priority_score:
                    highest_priority_score = priority_score
                    selected_box = detection
            
            raw_yaw_err, kf_yaw_est, yaw_cmd, yaw_i_term = 0.0, yaw_kalman_filter.get_estimated_state()[0], 0.0, yaw_pid_controller.I_term
            raw_pitch_align_err, kf_pitch_align_est, pitch_cmd, pitch_i_term = 0.0, pitch_kalman_filter.get_estimated_state()[0], 0.0, pitch_align_pid_controller.I_term
            tracked_class_id = -1

            if selected_box is not None:
                x1, y1, x2, y2 = selected_box['xyxy']
                raw_x_center_bbox, raw_y_center_bbox = (x1 + x2) / 2, (y1 + y2) / 2 
                ...
                yaw_error, pitch_error_align = estimated_x_center - image_center_x, estimated_y_center - image_center_y
                
                if abs(yaw_error) > DEAD_ZONE_YAW: yaw_rate_cmd = yaw_pid_controller.update(yaw_error, dt)
                else: yaw_rate_cmd = 0.0; yaw_pid_controller.reset()
                if abs(pitch_error_align) > DEAD_ZONE_PITCH_ALIGN: pitch_rate_cmd = pitch_align_pid_controller.update(pitch_error_align, dt)
                else: pitch_rate_cmd = 0.0; pitch_align_pid_controller.reset()
                ...
                for detection in detections:
                    ...
                    if 'conf' in detection:
                        cv2.putText(frame, f"{detection['conf']:.2f}", (int(dx1), int(dy1-5)), 
                                  cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
                cv2.circle(frame, (int(raw_x_center_bbox), int(raw_y_center_bbox)), 5, (0, 0, 255), -1)
                cv2.circle(frame, (int(estimated_x_center), int(estimated_y_center)), 8, (255, 0, 255), -1)
                cv2.line(frame, (int(image_center_x), 0), (int(image_center_x), image_height), (255, 0, 0), 2)
                cv2.line(frame, (0, int(image_center_y)), (image_width, int(image_center_y)), (255, 0, 0), 2)
                cv2.putText(frame, f"Tracking ID: {tracked_class_id}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
                cv2.putText(frame, f"Yaw Err: {yaw_error:.2f}px | Cmd: {yaw_rate_cmd:.1f}deg/s", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(frame, f"Pitch Err: {pitch_error_align:.2f}px | Cmd: {pitch_rate_cmd:.1f}deg/s", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(frame, f"Altitude: {current_altitude_m:.2f}m / Target: {2.0:.2f}m", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
            else:
                estimated_x_center, _ = yaw_kalman_filter.get_estimated_state()
                estimated_y_center, _ = pitch_kalman_filter.get_estimated_state()
                yaw_error, pitch_error_align = estimated_x_center - image_center_x, estimated_y_center - image_center_y
                ...
                raw_yaw_err, kf_yaw_est, yaw_cmd, yaw_i_term = 0.0, estimated_x_center, yaw_rate_cmd, yaw_pid_controller.I_term
                raw_pitch_align_err, kf_pitch_align_est, pitch_cmd, pitch_i_term = 0.0, estimated_y_center, pitch_rate_cmd, pitch_align_pid_controller.I_term
                tracked_class_id = -1
                
                cv2.circle(frame, (int(estimated_x_center), int(estimated_y_center)), 8, (255, 165, 0), -1)
                cv2.line(frame, (int(image_center_x), 0), (int(image_center_x), image_height), (255, 0, 0), 2)
                cv2.line(frame, (0, int(image_center_y)), (image_width, int(image_center_y)), (255, 0, 0), 2)
                cv2.putText(frame, "No Target Detected", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
                cv2.putText(frame, f"Yaw Err: {yaw_error:.2f}px | Cmd: {yaw_rate_cmd:.1f}deg/s", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(frame, f"Pitch Err: {pitch_error_align:.2f}px | Cmd: {pitch_rate_cmd:.1f}deg/s", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
                cv2.putText(frame, f"Altitude: {current_altitude_m:.2f}m / Target: {2.0:.2f}m", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)

            # Display TensorRT status
            engine_status = "TensorRT: ON" if _use_real_yolo_local else "TensorRT: OFF (Mock)"
            cv2.putText(frame, engine_status, (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
            cv2.putText(frame, "Tracking: ON", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        else:
            if is_offboard_active_from_script:
                try: await drone.offboard.stop(); print("Offboard mode stopped."); is_offboard_active_from_script = False
                except OffboardError as error: print(f"Failed to stop offboard: {error}")
            
            await send_attitude_rate_commands(drone, 0, 0, 0, HOVER_THRUST_VALUE)
            ...
            raw_yaw_err, kf_yaw_est, yaw_cmd, yaw_i_term = 0.0, image_center_x, 0.0, 0.0
            raw_pitch_align_err, kf_pitch_align_est, pitch_cmd, pitch_i_term = 0.0, image_center_y, 0.0, 0.0
            tracked_class_id = -1
            
            cv2.putText(frame, f"Tracking: OFF ({current_flight_mode.name} mode)", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(frame, f"Altitude: {current_altitude_m:.2f}m / Target: {2.0:.2f}m", (10, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)

        log_data_row.extend([raw_yaw_err, kf_yaw_est, yaw_cmd, yaw_i_term, raw_pitch_align_err, kf_pitch_align_est, pitch_cmd, pitch_i_term, tracked_class_id, current_altitude_m])
        csv_writer.writerow(log_data_row)
        if video_writer is not None: video_writer.write(frame)
        cv2.imshow("TensorRT Object Tracking", frame)
        if cv2.waitKey(1) & 0xFF == ord('q'): break

    print("Stopping and landing the drone.")
    if is_offboard_active_from_script:
        try: await drone.offboard.stop(); print("Offboard mode stopped before landing.")
        except OffboardError as error: print(f"Failed to stop offboard before landing: {error}")
    await drone.action.land(); await asyncio.sleep(5)
    await drone.action.disarm()
    cap.release(); video_writer.release() if video_writer is not None else None
    csv_file.close()

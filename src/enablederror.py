...
# Heartbeat Configuration (for Lua failsafe compatibility)
HEARTBEAT_ENABLED = True
HEARTBEAT_PARAM = 'SCR_USER1'  # Parameter name (must match Lua script)
...
# --- Heartbeat Task ---
async def heartbeat_task(drone):
    """
    Continuously send heartbeat to FC for Lua failsafe monitoring.
    Updates SCR_USER1 parameter with current timestamp.
    """
    ...
    while not EMERGENCY_STOP:
        try:
            ...
            if current_time - last_heartbeat >= HEARTBEAT_INTERVAL:
                # Set parameter to current timestamp
                await drone.param.set_param_float(HEARTBEAT_PARAM, float(current_time))
                last_heartbeat = current_time
            
            await asyncio.sleep(0.1)  # Check frequently but don't spam
...
async def tracking_loop():
    ...
    if HEARTBEAT_ENABLED:
        print(f"  Heartbeat: Active (updating {HEARTBEAT_PARAM} every {HEARTBEAT_INTERVAL}s)\n")
    else:
        print("")
    ...
    if HEARTBEAT_ENABLED:
        heartbeat_task_handle = asyncio.create_task(heartbeat_task(drone))
    ...
    try:
        while not EMERGENCY_STOP:
            ...
            if EMERGENCY_STOP:
                print("⚠ Emergency stop detected in main loop")
                break
        ...
        if heartbeat_task_handle and not heartbeat_task_handle.done():
            ...
            try:
                await asyncio.wait_for(heartbeat_task_handle, timeout=1.0)

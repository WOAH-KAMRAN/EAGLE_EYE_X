...
DEPTH_MODEL_TYPE = 'MiDaS_small'      # Currently supports 'midas' (more models can be added)
...
# Flag to signal display thread to stop
...
def display_thread_func():
    """
    Separate thread for displaying frames.
    This runs independently from the main async loop,
    so cv2.waitKey() blocking doesn't affect drone control.
    """
    ...
    # Optional: Enable OpenGL for better performance
    try:
        cv2.setWindowProperty("Object Tracking", 
                             cv2.WND_PROP_OPENGL, 
                             cv2.WINDOW_OPENGL)
        print("[Display Thread] OpenGL enabled")
    except:
        print("[Display Thread] OpenGL not available (that's okay)")
    
    while not display_should_stop.is_set():
        try:
            # Try to get a frame from the queue
            # Timeout of 0.1 seconds (100ms) prevents thread from hanging
            frame = display_queue.get(timeout=0.1)
            
            # Check if we got the stop signal (None = sentinel value)
            if frame is None:
            ...
            # Check for key press (this blocking call is OK here)
            key = cv2.waitKey(1) & 0xFF
            
            # If 'q' pressed, signal main loop to quit
            if key == ord('q'):
                print("[Display Thread] 'q' pressed, signaling main loop to stop")
                display_should_stop.set()  # Tell main loop to stop
                break
            
            # Optional: Add other key handlers
            # if key == ord('r'):  # Example: 'r' to reset something
            #     print("[Display Thread] Reset requested")
                
        except queue.Empty:
            # No frame available in queue
            # Just refresh the window and try again
            cv2.waitKey(1)
            continue
    ...
    # Cleanup
    print("[Display Thread] Closing window...")
...
async def tracking_loop():
    ...
    display_thread = threading.Thread(target=display_thread_func,
                                      daemon=True, name="display_thread")
    ...
    while True:
        ...
        if current_flight_mode == FlightMode.OFFBOARD:
            ...
            if not is_offboard_active_from_script:
                try:
                    ...
                    try:
                        display_queue.put_nowait(frame.copy())  # .copy() is important!
                    except queue.Full:
                        pass
        ...
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
        await asyncio.sleep(0.001) # Small sleep to allow other tasks to run
    ...
    # Send stop signal through queue (sentinel value)
    try:
        display_queue.put(None, timeout=1.0)
    except queue.Full:
        pass
    
    # Wait for display thread to finish (max 2 seconds)
